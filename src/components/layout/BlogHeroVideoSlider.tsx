...
import { useBlogHeroVideos } from '@/hooks/landing/useBlogHeroVideos';

interface BlogHeroVideoSliderProps {
  onVideosReady?: (count: number) => void;
}

export default function BlogHeroVideoSlider({ onVideosReady }: BlogHeroVideoSliderProps) {
  const { data: videos = [], isLoading, error } = useBlogHeroVideos();
  ...
  const handleVideoError = useCallback((index: number, e: React.SyntheticEvent<HTMLVideoElement, Event>) => {
    console.error(`Error loading blog video ${index}:`, e);
    setVideoErrors((prev) => {
      const newSet = new Set(prev);
      newSet.add(index);
      return newSet;
  ...
  const executeTransition = useCallback((targetIndex: number) => {
    ...
    targetVideo.play().catch((error) => {
      console.error(`Error playing blog video ${targetIndex}:`, error);
    });
  ...
  useEffect(() => {
    ...
    if (currentVideo && !videoErrors.has(currentIndex)) {
      currentVideo.play().catch((error) => {
        console.error(`Error playing blog video ${currentIndex}:`, error);
      });
    }
  ...
  if (shouldShowFallback) {
    return (
      <div className="absolute inset-0 w-full h-full z-[1] transition-opacity duration-500 opacity-100">
        <Image
          src="/figma-assets/massage-therapy.png"
